...
const api_key = 'RGAPI-653219d7-1acb-40ef-871a-38cda6fb53a6'
...
router.get('/:summoner', async function(req, res, next) {
  ...
  console.log("*")
  console.log("*")
  const puuid = await axiosAmericas.get(`/riot/account/v1/accounts/by-riot-id/${req.params.summoner}/NA1?api_key=${api_key}`)
  ...
  const summonerInfo = await axiosNA1.get(`/summoner/v1/summoners/by-puuid/${puuidResolved}?api_key=${api_key}`);
  // console.log("*")
  // console.log("*")
  // console.log("*")
  ...
  // console.log("*")
  const count = 10
  const matches = await axiosAmericas.get(`/tft/match/v1/matches/by-puuid/${puuidResolved}/ids?count=${count}&api_key=${api_key}`)
  .then(async e => {
    const fullInfoList = await Promise.all(
      e.data.map(async match => {
        // console.log("match: ", match)
        const res = await axiosAmericas.get(`/tft/match/v1/matches/${match}?api_key=${api_key}`)
